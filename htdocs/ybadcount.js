...
if(AdWordsApp.getExecutionInfo().isPreview())
    mailText.push('/!\\ This script was executed in preview mode. No real changes have been made.');

updateAdsIfNeededAndGetResults().each(function (item, action) {
    mailText.push('-> <b>' + item.length+ ' ads have been '+action+'d</b>');
});

var spreadsheet = spreadsheetReport();

if(spreadsheet) {
    //Permission
    spreadsheet.addEditors(MAIL_TO.split(/[,;]+/));

    var what = TAKE_ACTION ? 'a list of the applied' : 'the recommended';
    mailText.push('A new spreadsheet containing '+what+' actions has been created : \n'+spreadsheet.getUrl());
}

if(mailText) {
    var mailTitle = (AdWordsApp.getExecutionInfo().isPreview()?'[PREVIEW] ':'')+TITLE.split('/').pop();
    Logger.sendEmail(MAIL_TO, mailTitle, mailText, {
        name: 'AdQuality Script Bot',
        replyTo: 'gael@adquality.net',
        logo: 'http://www.adquality.otakudesign.fr/wp-content/uploads/2016/03/LOGO_picto-Adquality.png',
        footer: 'Best regards, \nAdQuality Team'
    });
}
...
/*
 *  The main predicate to automated triggered actions
 */
function actionToTakeFor(item, adGroupItem){
    if(_.isNull(item.adCount))
        return null;
    if(!item.isPaused && notEnoughHurriyetAds(item))
        return "pause"
    else if(!notEnoughHurriyetAds(item) && adGroupItem.containsOnlyPausedAds && !adGroupItem.isPaused)
        return "enable";

    return false;
}
function notEnoughHurriyetAds(item) {
    return item.adCount < MIN_AD_COUNT;
}


/*
 *  Update ads if needed and return a collection of results
 */
function updateAdsIfNeededAndGetResults () {
    return AdGroups().subCollection("Ads").trigger(function (adItem) {
        var action = actionToTakeFor(adItem, adItem._parent);

        if(TAKE_ACTION && _.isString(action)) {
            var collected = {};
            collected[action] = adItem.ad;
            return collected;
        }
    });
}
 ...
 *  Spreadsheet Report
 */
function spreadsheetReport () {
    var sheetOptions = {
        columns : ["Campaign // Ad Group","Ad Count","URL"],
        row : function (actionToTake, adItem, adGroupItem) {
            return [adGroupItem.campaignName + " // " +adGroupItem.name, adItem.adCount, adItem.url];
        }
    };

    var newSheetForAction = function (action) {
        return _.defaults({
            title : action ? action.toUpperCase()+"D" : "ND",
            row : function (adItem) {
                var actionToTake = actionToTakeFor(adItem, adItem._parent);
                if(actionToTake === action)
                    return sheetOptions.row(actionToTake,adItem, adItem._parent);
            }
        },sheetOptions);
    }

    return AdGroups().subCollection("Ads").slice(50000,0).toNewGoogleSheet(
        TITLE, newSheetForAction('pause'), newSheetForAction('enable'), newSheetForAction(null)
    );
}
...
function updateCache() {
    return Cache().post({
        ...
        urls: Ads().pluck("url"),
        // Xpath
        xpath: '//div[@class="fl count-title"]/strong/text()',
        // Regexp used to clean the content
        // needs escaping
        clean: "\\.|ilan -"
    });
}
...
function filterSelector(selector) {
    if (EXCLUDE_CAMPAIGNS.length) {
        selector = selector.withCondition("CampaignName NOT_IN ['" + EXCLUDE_CAMPAIGNS.join("','") + "']");
    }

    return selector;
}

function linkAdsToAdsGroup() {
    AdGroups().addRelationship(Ads, "adGroupId")
        .addProperties({
            containsOnlyPausedAds: function(item) {
                var passed = true;
                item.Ads.find(function (adItem) {
                    if(!adItem.isPaused) {
                        passed = false;
                        return true;
                    }
...
/*
 *  AdWords AdGroups selector
 */
function activeCampaignsSelector() {
...
function Campaigns() {
    return Singleton(function () {
        return new Collection( activeCampaignsSelector, function(adwordsCampaign) {
            return { campaign: adwordsCampaign }
        })
    },function (collection) {
        return collection.addProperties({
            name: function(item) {
                return item.campaign.getName()
            }
        });
    });
}
 ...
 *  AdWords AdGroups selector
 */
function adGroupsInActiveCampaignsSelector() {
...
/*
 *  Fetching all needed entities in a wrapper
 *
 */
function AdGroups() {
    return Singleton(function () {
        return new Collection( adGroupsInActiveCampaignsSelector, function(adWordsAdGroup) {
            return { adGroup: adWordsAdGroup }
        })
    },function (collection) {
        return collection.addProperties({
            isPaused : function (item) {
                return item.adGroup.isPaused();
            },
            campaignName: function(item) {
                return item.adGroup.getCampaign().getName()
            },
            name: function(item) {
                return item.adGroup.getName()
            },
            id : function (item) {
                return item.adGroup.getId();
            }
        });
    });
}
...
function adsInActiveCampaignsSelector() {
    var selector = AdWordsApp.ads()
        .withCondition('CampaignStatus = ENABLED')
        .withCondition('Type NOT_IN  [MOBILE_AD, MOBILE_IMAGE_AD]');
...
function Ads() {
    return Singleton(function() {
        ...
        return new LargeCollection(adsInActiveCampaignsSelector, function (adWordsAd) {
            return { ad: adWordsAd }
        }).collectUsing('CampaignName', Campaigns(), 'name', 10);

    }, function (collection) {

        return collection.addProperties({
                isPaused : function (item) {
                    return item.ad.isPaused();
                },
                url: function(item) {
                    var url = item.ad.urls().getFinalUrl();
                    if (url) {
                        url = url.toString().replace(DOMAIN,'');
                        if(url.indexOf('{') >= 0) {
                ...
                adCount: function(item) {
                    var countCrawl = Cache().find({
                        url: item.url
                    });

                    if (_.isUndefined(countCrawl) || countCrawl == "") {
                        //missing[item.url] = "";
                        return null;
                    }

                    return parseInt(countCrawl) || 0;
                },
                adGroupId : function (item) {
                    return item.ad.getAdGroup().getId();
                }
            })
            .reject(function(item) {
                return !item.url || item.url.match(/hurriyetemlak\.com\/?$/)
            });
...
/*
 *  Parallelize urls crawl by using a distant Scrapy server
 *  The current script benefits data from previous execution
 */
function Cache() {
    return Singleton(function () {
        return new Scrapy("https://adq-parser-zzgael.c9users.io/ybadcount/")
    });
}

function debug () {
    Logger.log(_.size(missing)+ " URLs missing in cache");
    Logger.log(missing);


    /*var i=0,j=0;
     AdGroups().subCollection("Ads").each(function (item) {
     if(actionToTakeFor(item, item._parent))
     i++;
     });
     Logger.log([i,j]);*/
}
