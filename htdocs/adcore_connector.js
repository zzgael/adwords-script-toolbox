...
this.AdGroupsCollection = function() {
  return Collector({
    ...
    props: {
      ...
      isEnabled: function(item) {
        return item.adGroup.isEnabled();
      }
    },
    methods: {
      pause: function(item) {
        return item.adGroup.pause();
      },
      enable: function(item) {
        return item.adGroup.enable();
      },
      hasPendingState: function() {
        return this.find(function(item) {
          return item.pendingState !== null;
        });
      },
      commitAllPendingUpdates: function() {
        log("Committing AdGroups pending states");
        if (!this.hasPendingState()) {
          warning("No pending state to commit.");
        }
        return this.each(function(item) {
          if (item.pendingState === null) {
            return;
          }
          ...
          item.isEnabled = item.pendingState;
          item.pendingState = null;
          return item.stateHasChanged = true;
        });
      },
      setPendingState: function(item, newState) {
        return item.pendingState = newState;
      }
    },
    relations: [['AdCoreAdsCollection', 'adGroupId']],
    assertions: [
      function() {
        var adsItems;
        adsItems = this.subCollection("AdCoreAdsCollection").get();
        return expect(adsItems).to.be.an("array")["with"].length.above(0);
      }
    ]
  });
};

this.AdCoreAdsCollection = function() {
  return Collector({
    selector: withinCampaigns(AdWordsApp.ads().withCondition('Headline IN ["' + _ADCORE_CONNECTOR_TITLE.join('","') + '"]')),
    key: 'ad',
    props: {
      headline: "Headline",
      productId: "Description1",
      adGroupId: "AdGroup.Id"
    },
    methods: {
      getProduct: function(item) {
        return item.FeedItemsCollection.get(0);
      }
    },
    relations: [['FeedItemsCollection', 'id', 'productId'], ['KeywordsCollection', 'adGroupId', 'adGroupId']]
  });
};

this.KeywordsCollection = function() {
  return Collector({
    selector: function() {
      return withinCampaigns(AdWordsApp.keywords().withCondition('Status="ENABLED"').withCondition('AdGroupStatus="ENABLED"').withCondition('CampaignStatus="ENABLED"'));
    },
    largeDivider: ["CampaignName", CampaignsCollection, null, 20],
    key: 'keyword',
    props: {
      adGroupId: "AdGroup.Id",
      text: "Text"
...
this.FeedsCollection = function() {
  return Collector({
    selector: _FEEDS_CAMPAIGNS,
    init: function(campaignsList, feedUrl) {
      return {
        feedItemsCollection: new Collection(getFeedItems(_PROXY_URL + _BASE_SRC + feedUrl)).addProperties({
          id: function(item) {
            return capitalizeWords(item.getChildVal('id'));
          },
          stock: function(item) {
            return item.getChildVal('Stock');
          },
          prixMini: function(item) {
            return item.getChildVal('PrixMini');
          }
        }),
    ...
    assertions: [
      function() {
        return expect(this.get()).to.be.an("array")["with"].length.above(0);
      }, function() {
        return expect(this.get(0)).to.be.an("object")["with"].property('feedItemsCollection');
      }, function() {
        return expect(this.get(0).campaignsList[0]).to.have.string("FR -");
      }
    ]
  });
};

this.FeedItemsCollection = function() {
  return Collector({
    selector: _.flatten(_.map(FeedsCollection().pluck('feedItemsCollection'), function(feedItemsCollection) {
      return feedItemsCollection.get();
    })),
    assertions: [
      function() {
        return expect(this.get()).to.be.an("array")["with"].length.above(0);
...
this.CampaignsCollection = function() {
  return Collector({
    selector: _.flatten(FeedsCollection().pluck("campaignsList"))
  });
};

this.withinCampaigns = function(selector) {
  return selector.withCondition('CampaignName IN ["' + this.CampaignsCollection().get().join('","') + '"]');
};

this.getFeedItems = function(feedURL) {
  var childVal, document, filter, items, products, resp;
  log('Fetching ' + feedURL);
  resp = UrlFetchApp.fetch(feedURL);
  childVal = function(item, f) {
    var child;
    child = item.getChildren(f);
    if (child) {
  ...
  filter = function(item) {
    if (_USE_FILTERS) {
      _.each(FILTERS, function(feedProperty) {
        if (_FILTERS[feedProperty] !== '' && _FILTERS[feedProperty] !== childVal(item, feedProperty)) {
          return false;
        }
      });
    }
  ...
  if (resp.getResponseCode() === 200) {
    document = XmlService.parse(resp.getContentText());
    items = document.getRootElement().getChildren('items');
    log('Initializing ' + items.length + ' feed items.');
    products = [];
    _.each(items, function(item) {
      if (filter(item)) {
        return products.push({
          xmlItem: item,
          getChildVal: function(f) {
            return childVal(item, f);
          }
        });
      }
    });
    ...
    throw 'An error occured while trying to fetch: ' + feedURL;
  }
  return null;
};
...
/*
 *  AdCore Connector v3
 *  AdQuality/IDMKR/Gaël Debost
 */
var _PROXY_URL, _USE_FILTERS, attachments, detailedLog, mailBody, updateLog, updatedAdGroupsCollection, updatedKeywordsCollection;

_PROXY_URL = 'https://idmkr.io/adquality/CentralePneus/redirection-flux.php?f=';

_USE_FILTERS = _.size(_FILTERS);

Logger.setLevel(3);

AdGroupsCollection().subCollection("AdCoreAdsCollection").progress(function(adItem) {
  var adGroupItem, product, productHasEnoughStock;
  product = AdCoreAdsCollection().getProduct(adItem);
  if (!product) {
    notice("No feed item found for ad id " + adItem.productId);
    return;
  }
  productHasEnoughStock = product.stock > _STOCK_LIMIT_AD_PAUSED;
  adGroupItem = adItem._parent;
  if (adGroupItem.isEnabled && !productHasEnoughStock) {
    AdGroupsCollection().setPendingState(adGroupItem, false);
  } else if (!adGroupItem.isEnabled && productHasEnoughStock) {
    AdGroupsCollection().setPendingState(adGroupItem, true);
  }
  if (adGroupItem.pendingState !== null) {
    adGroupItem.updateLogInfo = "-- " + (AdGroupsCollection().getType()) + "  " + adGroupItem.name + " - " + (adGroupItem.pendingState ? "enabled" : "paused") + " (Stock: " + product.stock + ")";
    return notice(adGroupItem.updateLogInfo);
  } else {
    return notice("-- " + AdGroupsCollection().getType() + " " + adGroupItem.name + "(Stock: " + product.stock + ')');
  }
}, function(percentage, i, total) {
  if (percentage === 0) {
    log("Starting adGroups processing");
  }
  if (percentage % 20 === 0) {
    return log(percentage + "% (" + i + "/" + total + ") - Pending adGroups updates: " + AdGroupsCollection().where({
      pendingState: true
    }).size());
  }
});

AdGroupsCollection().commitAllPendingUpdates();

AdGroupsCollection().subCollection("AdCoreAdsCollection").progress(function(adItem) {
  var adGroupItem, product;
  product = AdCoreAdsCollection().getProduct(adItem);
  if (product) {
    adGroupItem = adItem._parent;
    return adItem.KeywordsCollection.each(function(keywordItem) {
      keywordItem.keyword.setAdParam(1, product.prixMini);
      keywordItem.paramsUpdated = true;
      return keywordItem.updateLogInfo = "-- AdGroup  " + adGroupItem.name + " - Keyword " + keywordItem.text + "  - Params { 1, " + product.prixMini + " }";
    });
  }
}, function(percentage, i, total) {
  if (percentage === 0) {
    log("Starting Ad Params update.");
  }
  if (percentage % 20 === 0) {
    return log(percentage + "% (" + i + "/" + total + ") - Params updated : " + KeywordsCollection().where({
      paramsUpdated: true
    }).size());
  }
});

updatedAdGroupsCollection = AdGroupsCollection().where({
  stateHasChanged: true
});

updatedKeywordsCollection = KeywordsCollection().where({
  paramsUpdated: true
});

log([
  "\nGroupe d'annonces modifiés :" + updatedAdGroupsCollection.size(), "\nGroupe d'annonces inchangés :" + AdGroupsCollection().where({
    stateHasChanged: false
  }).size(), "\nGroupe d'annonces (connus) en pause : " + AdGroupsCollection().where({
    isEnabled: true
  }).size(), "\nGroupe d'annonces (connus) activés : " + AdGroupsCollection().where({
    isEnabled: false
  }).size(), "\nParamètres de mots clés mis à jour : " + updatedKeywordsCollection.size(), "\nParamètres de mots clés inchangés : " + KeywordsCollection().where({
    paramsUpdated: false
  }).size() + "\n", "\nNombre de produits tous flux : " + FeedItemsCollection().size() + "\n", "\nProduits de flux non trouvés : " + AdCoreAdsCollection().filter(function(i) {
    return this.getProduct(i);
  }).size() + "\n"
]);

mailBody = "\n- Nombre de groupes d'annonces dont l'état a été modifié : " + (updatedAdGroupsCollection.size()) + "\n- Nombre de paramètres mis à jour : " + (updatedKeywordsCollection.size()) + "\n\n";

attachments = [];

if (updatedAdGroupsCollection.size()) {
  updateLog = updatedAdGroupsCollection.map(function(adGroupItem) {
    return adGroupItem.updateLogInfo;
  });
  detailedLog = "Détail des groupes d'annonces mis à jour : \n" + updateLog.join("\n");
  attachments.push(Utilities.newBlob(detailedLog, 'text/plain', 'detailed_adgroup_log.txt'));
}

if (updatedKeywordsCollection.size()) {
  updateLog = updatedKeywordsCollection.map(function(keyword) {
    return keyword.updateLogInfo;
  });
  detailedLog = "Détail des paramètres d'annonces mis à jour : \n" + updateLog.join("\n");
  attachments.push(Utilities.newBlob(detailedLog, 'text/plain', 'detailed_params_log.txt'));
}

if (_EMAIL_TO) {
  Logger.sendEmail(_EMAIL_TO, "[AdWords Script] " + updatedAdGroupsCollection.size() + " Groupes d'Annonces & " + KeywordsCollection().where({
    paramsUpdated: true
  }).size() + " Paramètres impactés", mailBody, {
